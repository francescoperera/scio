...
trait AnnoyUri extends Serializable {
  val path: String
  private[annoy] def getReader(metric: ScioAnnoyMetric, dim: Int, nTrees:Int): AnnoyReader
...
private class LocalAnnoyUri(val path: String) extends AnnoyUri {
  ...
  override private[annoy] def getReader(metric: ScioAnnoyMetric, dim: Int, nTrees: Int): AnnoyReader
  = new AnnoyReader(metric, dim, nTrees, path)
...
private class RemoteAnnoyUri(val path: String, options: PipelineOptions) extends AnnoyUri {
  ...
  override private[annoy] def getReader(metric: ScioAnnoyMetric, dim: Int, nTrees:Int)
  : AnnoyReader = {
    val localPath = rfu.download(new URI(path))
    new AnnoyReader(metric, dim, nTrees, localPath.toString())
  }
  override private[annoy] def saveAndClose(w: AnnoyWriter): Unit = {
