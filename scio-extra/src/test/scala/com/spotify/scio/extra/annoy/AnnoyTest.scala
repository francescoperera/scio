...
class AnnoyTest extends PipelineSpec {
  ...
  "SCollection" should "support .asAnnoy with specified local file" in {
    val sc = ScioContext()
    val metric = Angular
    val dim = 2
    val nTrees = 10
    val p = sc.parallelize(sideData).asAnnoy(metric, dim, nTrees, "test.tree").materialize
    sc.close().waitUntilFinish()
  ...
  it should "throw exception when Annoy file already exists" in {
    ...
    val path = tmpDir + "test.tree"
    val index = new File(path)
    val annoy = Files.touch(index)
    // scalastyle:off no.whitespace.before.left.bracket
    the [IllegalArgumentException] thrownBy {
      runWithContext {
        _.parallelize(sideData).asAnnoy(Angular, 40, 10, path)
      }
